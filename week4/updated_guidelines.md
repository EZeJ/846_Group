...
Details in `week4/resources/updated_guideline_4/`
...
In the initial Guideline 6 it was stated that CAPITALIZATION of the roles is supposed to make the results better. We tested this guideline on the corner case, where the conversation included a lot of the capitalized words, as they were SQL-related. 

In the result, the capitalization of the roles didn't provide the expected results, and we came up with an updated guideline for this specific cases: it is better to apply JSON formatting for specifying where is the role and where is the text, as then no formatting misunderstanding is in the text. 

In the resulting outputs, JSON formatting of the conversation allowed the model to better separate which speaker had which points.

LOGS:

**USING CHAT FORMATTING "0" (None):**
...
**USING CHAT FORMATTING "1" (Capitalization):**
...
As it is visible, the JSON formatted inout file gave far better response. Details can be found in `week4/resources/updated_guideline_6`
...
## Solution or Output generated by GitHub Copilot
Details in `week4/resources/updated_guideline_3/copilot_ouput_and_analysis.md`
...
## Solution or Output generated by GitHub Copilot
Details in `week4/resources/updated_guideline_2`
